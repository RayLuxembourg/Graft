...
- `ask --source`: the #1 hit gets an 80-line window (others 40), and a span over its cap shows its head AND its last 10 lines with the omitted range named — a definition's decision (a `return`, a URL, an AngularJS `canExecute`) sits at its end; a head-only window hid `dashboards.export_pdf` at L66 of a L4-L70 command in four benchmark runs.
- Unscoped `grep` prefilters through the store's FTS index: the files whose token bags carry every word of the pattern are the only ones opened and scanned (`fields/search` over 66 repos: 923 candidate files, 2.4 s, 214 MB — was ~4 min reading 47k files). The CLI prints how many files were scanned so a zero-hit result is never silent.
- SQLite store: `graft build` also writes `.graph/graph.sqlite` (nodes, edges, ask sidecar docs and term frequencies, an FTS5 inverted index over the token bags) with Node's built-in `node:sqlite`. `ask --in` reads the scope's rows; unscoped `ask` reads only the best 4,000 FTS5 candidates for the query's tokens plus their neighbours, with global IDF; `callers` reads the symbol's neighbourhood by BFS; `skeleton` reads one file's rows. The JSON files remain and answer when the store is absent or older than `wiring.json`.
